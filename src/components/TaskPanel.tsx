    ...
    <div className="space-y-6 max-w-3xl mx-auto">
      ...
      <div className="bg-white dark:bg-slate-900 border border-slate-200/80 dark:border-slate-800 rounded-3xl p-6 sm:p-8 shadow-sm relative overflow-hidden">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 relative z-10">
          <div>
            <div className="flex items-center space-x-3 text-xs font-mono text-slate-900 dark:text-slate-100 mb-1 font-semibold">
              ...
              {userTodayStatus === 'not_done' && (
                <span className="px-2.5 py-0.5 rounded-full bg-slate-200 dark:bg-slate-800 text-slate-700 dark:text-slate-300 text-[11px] font-bold tracking-wider uppercase flex items-center space-x-1">
                  <XCircle className="w-3 h-3" />
                  <span>Not Done</span>
                </span>
              )}
              {!userTodayStatus && (
                <span className="px-2.5 py-0.5 rounded-full bg-slate-100 dark:bg-slate-800 text-slate-500 text-[11px] font-semibold tracking-wider uppercase">
                  Check-in Pending
                </span>
              )}
            </div>
          ...
          <div className="bg-slate-50 dark:bg-slate-800/80 border border-slate-200/80 dark:border-slate-700/60 rounded-2xl p-4 flex items-center space-x-3 shrink-0 shadow-xs">
            ...
            <div>
              ...
              <div className="text-xl font-black text-slate-900 dark:text-slate-100">
                {userStats.currentStreak} <span className="text-xs font-medium text-slate-400">days</span>
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="bg-white dark:bg-slate-900 border border-slate-200/80 dark:border-slate-800 rounded-3xl p-6 sm:p-8 shadow-sm space-y-6">
        <div className="flex items-center justify-between pb-4 border-b border-slate-100 dark:border-slate-800">
          <div className="flex items-center space-x-2.5">
            <span className="w-2.5 h-2.5 rounded-full bg-slate-900 dark:bg-slate-100 animate-pulse" />
          ...
          {currentUser.isAdmin && onOpenAdminPanel && (
            <button
              ...
              className="text-xs font-bold text-slate-900 dark:text-slate-100 hover:underline"
            >
              Edit Task (Admin)
            </button>
          )}
        ...
        {todayTask ? (
          <div className="space-y-6">
            <div className="p-5 bg-slate-50/80 dark:bg-slate-800/60 rounded-2xl border border-slate-200/80 dark:border-slate-700/80">
